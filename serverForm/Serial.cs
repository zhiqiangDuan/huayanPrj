...
using static serverForm.Hy_client;
...
namespace serverForm
{
    class Serial : HY_crc
    {
        ...
        public Serial(string portName)
        {
            ...
            serialPort.BaudRate = 4800;//波特率
            serialPort.DataBits = 8;//数据位
            serialPort.StopBits = StopBits.One;
            serialPort.Parity = Parity.None;
            string date = DateTime.Now.ToString("yyyy-MM-dd");
            if (wanyi(deadLine, date) < 0)
            {
                return;
            }
            try
        ...
        private void dataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            {
                int n = serialPort.BytesToRead; //记录缓存数据的长度
                byte[] readBuf = new byte[n]; //声明一个临时数组存储当前来的串口数据  
                serialPort.Read(readBuf, 0, n); //读取缓冲数据
                /*if (readCount != 11)//数据不全就丢掉
                {
                    LogHelper.WriteLog(2,"readData", readBuf);
                    return;
                }*/
                ...
                if (bufLength + n > 110)//收到的数据超过buf，将数据清空
                {
                    bufLength = 0;
                ...
                Array.Copy(readBuf, 0, msgBuf, bufLength, n);
                bufLength += n;
                if (bufLength < 11)
                    return;
                checkMsg();
                
                LogHelper.WriteLog(1, "[read]", readBuf);
        ...
        public void sendDataThread()
        {
            ...
            if (serialPort.IsOpen)
            {
                byte[] sendMsg = getCRC(sendBuf);
                LogHelper.WriteLog(1, "[send]", sendBuf);
                //WriteLog("write");
                printMsg("[send]", sendMsg);
                if (!(sendMsg[2] == 0x04 && sendMsg[3] == 0x0C && sendMsg[4] == 0x0))
                {
                    serialPort.Write(sendMsg, 0, 11);
                    Thread.Sleep(10);
                    serialPort.Write(sendMsg, 0, 11);
                    Thread.Sleep(10);
                    serialPort.Write(sendMsg, 0, 11);
